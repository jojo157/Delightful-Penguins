...
def artist_status(request):
    """
    This function updates the user status to Offline
    or Online depending on the status recieved
    """
...
def reply(request, id):
    """
    This function renders a page with the message details
    that the artist has choosen to reply to.
    """
    if request.user.is_superuser:
...
def artist_send_reply(request):
    ...
    The function updates the reply recieved field to True for the message
    that a reply was sent to.
    """
    if request.user.is_superuser:
